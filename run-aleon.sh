#!/bin/bash

################################################################################
# aleon-run.sh - Codespace Setup for open-webui
# Installs everything in /tmp to avoid workspace storage limits
################################################################################

set -e

WORK_DIR="/tmp/aleon-open-webui-workspace"
LOG_FILE="/tmp/aleon-setup.log"

# Цвета для вывода
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Функции логирования
log_info() { echo -e "${BLUE}[INFO]${NC} $*" | tee -a "$LOG_FILE"; }
log_success() { echo -e "${GREEN}[SUCCESS]${NC} $*" | tee -a "$LOG_FILE"; }
log_warn() { echo -e "${YELLOW}[WARN]${NC} $*" | tee -a "$LOG_FILE"; }
log_error() { echo -e "${RED}[ERROR]${NC} $*" | tee -a "$LOG_FILE"; }

################################################################################
# Проверка места на диске
################################################################################
check_storage() {
    log_info "Проверка свободного место..."
    
    WORKSPACE_SPACE=$(df /workspaces 2>/dev/null | awk 'NR==2 {print int($4/1024)}' || echo "N/A")
    TMP_SPACE=$(df /tmp | awk 'NR==2 {print int($4/1024)}')
    
    echo ""
    echo "📊 Доступное место:"
    echo "  /workspaces: ${WORKSPACE_SPACE}MB"
    echo "  /tmp: ${TMP_SPACE}MB"
    echo ""
    
    if [ "$TMP_SPACE" -lt 5000 ]; then
        log_error "/tmp имеет мало места (${TMP_SPACE}MB)"
        exit 1
    fi
}

################################################################################
# Подготовка рабочей директории
################################################################################
setup_workspace() {
    log_info "Создание рабочей директории в /tmp..."
    
    rm -rf "$WORK_DIR" 2>/dev/null || true
    mkdir -p "$WORK_DIR"
    
    # Копируем репозиторий (только исходники, без node_modules и .venv)
    log_info "Копирование исходников..."
    cd /workspaces/open-webui
    
    # Копируем файлы, кроме тяжелых директорий
    rsync -av \
        --exclude='node_modules' \
        --exclude='.venv' \
        --exclude='__pycache__' \
        --exclude='.git' \
        --exclude='dist' \
        --exclude='build' \
        . "$WORK_DIR/" 2>/dev/null || cp -r . "$WORK_DIR/"
    
    cd "$WORK_DIR"
    log_success "Рабочая директория готова: $WORK_DIR"
}

################################################################################
# Настройка Node.js
################################################################################
setup_nodejs() {
    log_info "Настройка Node.js..."
    
    # Проверяем версию Node
    if ! command -v node &> /dev/null; then
        log_error "Node.js не установлен"
        exit 1
    fi
    
    NODE_VERSION=$(node -v)
    NPM_VERSION=$(npm -v)
    log_success "Node.js версия: $NODE_VERSION"
    log_success "npm версия: $NPM_VERSION"
    
    # Настройка npm для установки в /tmp
    mkdir -p /tmp/npm-cache
    mkdir -p /tmp/npm-global
    
    npm config set cache /tmp/npm-cache
    npm config set prefix /tmp/npm-global
    
    # Установка зависимостей
    log_info "Установка npm зависимостей (это займет время)..."
    npm ci --prefer-offline --no-audit --legacy-peer-deps 2>&1 | tee -a "$LOG_FILE"
    
    log_success "npm зависимости установлены"
}

################################################################################
# Настройка Python
################################################################################
setup_python() {
    log_info "Настройка Python окружения..."
    
    if ! command -v python3 &> /dev/null; then
        log_error "Python3 не установлен"
        exit 1
    fi
    
    PYTHON_VERSION=$(python3 --version)
    log_success "$PYTHON_VERSION"
    
    # Создаем виртуальное окружение в /tmp
    VENV_PATH="/tmp/aleon-venv"
    rm -rf "$VENV_PATH" 2>/dev/null || true
    
    log_info "Создание виртуального окружения в $VENV_PATH..."
    python3 -m venv "$VENV_PATH"
    source "$VENV_PATH/bin/activate"
    
    log_info "Обновление pip, setuptools, wheel..."
    pip install --upgrade pip setuptools wheel --cache-dir /tmp/pip-cache 2>&1 | tail -5
    
    # Проверяем requirements.txt
    if [ -f "$WORK_DIR/requirements.txt" ]; then
        log_info "Установка Python зависимостей из requirements.txt..."
        pip install -r requirements.txt --cache-dir /tmp/pip-cache 2>&1 | tee -a "$LOG_FILE"
    elif [ -f "$WORK_DIR/pyproject.toml" ]; then
        log_info "Установка Python зависимостей из pyproject.toml..."
        pip install -e . --cache-dir /tmp/pip-cache 2>&1 | tee -a "$LOG_FILE"
    fi
    
    log_success "Python зависимости установлены"
    
    # Сохраняем путь для активации
    echo "$VENV_PATH" > /tmp/aleon-venv-path.txt
}

################################################################################
# Сборка приложения
################################################################################
build_app() {
    log_info "Сборка приложения..."
    
    if [ -f "package.json" ]; then
        log_info "Запуск npm build..."
        npm run build 2>&1 | tee -a "$LOG_FILE" || log_warn "npm build может не работать в dev окружении"
    fi
    
    log_success "Сборка завершена"
}

################################################################################
# Создание стартового скрипта
################################################################################
create_startup_script() {
    local START_SCRIPT="/tmp/aleon-start.sh"
    
    cat > "$START_SCRIPT" << 'STARTUP_EOF'
#!/bin/bash

VENV_PATH=$(cat /tmp/aleon-venv-path.txt 2>/dev/null || echo "/tmp/aleon-venv")
WORK_DIR="/tmp/aleon-open-webui-workspace"

# Активируем Python окружение
source "$VENV_PATH/bin/activate" 2>/dev/null || true

# Добавляем npm global в PATH
export PATH="/tmp/npm-global/bin:$PATH"

cd "$WORK_DIR"

echo "🚀 Запуск open-webui..."
echo "   Директория: $WORK_DIR"
echo "   Python venv: $VENV_PATH"
echo ""

# Запуск в зависимости от конфигурации
if [ -f "run.sh" ]; then
    bash run.sh
elif [ -f "run-compose.sh" ]; then
    bash run-compose.sh
else
    # Стандартный запуск
    if command -v npm &> /dev/null; then
        npm run dev
    else
        python3 -m uvicorn backend.main:app --host 0.0.0.0 --port 8000
    fi
fi
STARTUP_EOF
    
    chmod +x "$START_SCRIPT"
    log_success "Стартовый скрипт создан: $START_SCRIPT"
}

################################################################################
# Вывод информации для запуска
################################################################################
print_summary() {
    cat << EOF

╔════════════════════════════════════════════════════════════════╗
║           ✅ Установка завершена успешно!                      ║
╚════════════════════════════════════════════════════════════════╝

📁 Директория установки: $WORK_DIR
📝 Логи: $LOG_FILE

🚀 Для запуска открой новый терминал и выполни:

    /tmp/aleon-start.sh

Или вручную:

    source /tmp/aleon-venv/bin/activate
    export PATH="/tmp/npm-global/bin:\$PATH"
    cd $WORK_DIR
    npm run dev

💾 Основное место используется: /tmp
   Workspace чистый!

ℹ️  Очистка (если нужно):
   rm -rf $WORK_DIR
   rm -rf /tmp/aleon-venv
   rm -rf /tmp/npm-cache
   rm -rf /tmp/npm-global

EOF
}

################################################################################
# Главная функция
################################################################################
main() {
    echo ""
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║  Aleon's open-webui Codespace Setup (установка в /tmp)       ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo ""
    
    check_storage
    setup_workspace
    setup_nodejs
    setup_python
    build_app
    create_startup_script
    print_summary
    
    log_success "Готово! Используй /tmp/aleon-start.sh для запуска"
}

# Запуск
main
